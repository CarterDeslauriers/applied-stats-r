...
```{r}
df = df %>% mutate(SEX = na_if(SEX, "Unknown"))
df = df %>% mutate(SEX = na_if(SEX, "Not Rep"))
```
...
```{r}
df = df %>% mutate(SEX = replace_na(SEX, names(which.max(table(SEX)))))
```
...
```{r}
df = df %>% mutate(AGE = str_replace(AGE, "Less than 1", "0"))
table(df$AGE)
```
...
df = df %>% mutate(TRAV_SP = str_remove(TRAV_SP, "MPH"))
df = df %>% filter(TRAV_SP != "Greater")
df = df %>% mutate(TRAV_SP = str_replace(TRAV_SP, "Stopped", "0"))
df = df %>% mutate(TRAV_SP = na_if(TRAV_SP, "Unknown"))
class(df$TRAV_SP)
df = df %>% mutate(TRAV_SP = as.numeric(TRAV_SP))
df = df %>% mutate(TRAV_SP = replace_na(TRAV_SP, median(TRAV_SP, na.rm = TRUE)))
...
```{r}
table(df$SEAT_POS)
drivers = df %>% filter(SEAT_POS == "Front Seat, Left Side")
