...
```{r}
df$weekend = ifelse(df$weekday %in% c(1, 7), 1, 0)
```
...
df %>%
  group_by(month2) %>%
  summarize(mean(positiveIncrease))
```
...
```{r}
df %>% 
  filter(date>"2020-10-01"&date<"2020-12-01"&weekend==1) %>% 
  group_by(month) %>% 
...
df %>% 
  ...
  group_by(weekend) %>% 
  summarise(avg_hospital = mean(hospitalized, na.rm=TRUE))
```
...
df2 %>% 
  filter(state=="Rhode Island"&date>="2021-01-01"&date<="2021-12-31") %>% 
  group_by(date) %>% 
...
df2 %>% 
  filter(state=="Rhode Island"&date>="2021-01-01"&date<="2021-12-31") %>% 
  group_by(weekdays) %>% 
  summarise(avg_cases = median(cases))

print("I did'nt difference here so you are getting the cumulative number")
...
```{r}
age_column = select(df3, Age)
```

```{r}
older = filter(df3, Age > 40)
```

```{r}
mutate(df3, age_months = Age * 12)
```

```{r}
summarise(df3, avg_fare = mean(Fare))
```

```{r}
arrange(df3, desc(Fare))
```
...
```{r}
arrange(count(df3, Embarked), desc(n))
```

```{r}
arrange(count(filter(df3, Survived == 1), Pclass), desc(n))
```

```{r}
summarise(group_by(df3, Pclass), avg_fare = mean(Fare, na.rm = TRUE))
```

```{r}
summarise(group_by(filter(df3, Sex == "female"), Pclass), rate = mean(Survived, na.rm = TRUE))
```

```{r}
arrange(summarise(group_by(filter(df3, Sex == "female"), Pclass), rate = mean(Survived, na.rm = TRUE)), desc(rate))
```
