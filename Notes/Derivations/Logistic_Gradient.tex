...
\section{Building Logistic Function}

% SET UP
\subsection{Data and Labels: $d$, $x_j$, $y$}
...
% MOTIVATION FOR Z
\subsection{Linear Score: $z$, $b$}

Our goal is simply take these $x_j$ and predict $y$ as best as possible.
So intuitively we can take a single observation call it $x_2$. Choose an arbitrary break
$x_2 > 1$ say. Let our prediction by $1$ for $y$ when $x_2 > 1$.
But this doesn't take into account all our observations. We want to take
into consideration all of our variables when making a decision. So let
z be the summation of the $d$ observations.

\begin{equation*}
z = x_1 + x_2 + \cdots + x_d + b
\end{equation*} 
...
\subsection{Input Weighting: $w_j$}
...
history might be more important than your hair color in predicting if you will
repay or not. 
...
But how do we get these weights? We need to conclude based on the current weights
how confident we are that $y = 1$ ie given our current value of z how confident are we
that this observation is a true $y = 1$. Well z is arbitrary, $w_1$ can be as big as we want,
z can be $100$, $-0.5$ so on; what does that mean?

% SIGMA(Z)
\subsection{Score to Probability: $\sigma(z)$}
...
To get a concept of a probability we must have the function $0 \leq p \leq 1$. We would
also like $p \to 0 \text{ as } z \to -\infty$ and $p \to 1 \text{ as } z \to \infty$.
...
Observe that since $e^{x} > 0$ always we have

\begin{equation*}
1 + e^{-z} > 1 \iff \frac{1}{1 + e^{-z}} < 1
\end{equation*}

and

\begin{equation*}
1 + e^{-z} > 0 \implies \frac{1}{1 + e^{-z}} > 0
\end{equation*}

Since denominator will always be greater than numerator and positive.

% PROOF FOR P LIMITS
\subsection{Limits of $p$}
...
\begin{align*}
e^{-z} < \epsilon
  &\iff -z < \log\!\left(\epsilon\right) \\
  &\iff z > -\log\!\left(\epsilon\right) \\
  &\iff z > \log\!\left(\frac{1}{\epsilon}\right)
\end{align*}

Meaning whenever $z \geq N$ where $N > \log\!\left(\frac{1}{\epsilon}\right)$ we have 
...
specifically choose $N > \log\!\left(\frac{1 - \epsilon}{\epsilon}\right)$
...
\begin{equation*}
\log\!\left(\frac{1 - \epsilon}{\epsilon}\right) < -z \iff -\log\!\left(\frac{1 - \epsilon}{\epsilon}\right) > z
\end{equation*} 

Meaning whenever $z \leq -N$ where $N > \log\!\left(\frac{1 - \epsilon}{\epsilon}\right)$ we have 
...
Now that we have proved the limit behavior of p and we have defined it as the probability
...
to the true value of $y=0$ or $y=1$. But an absolute difference 
doesn't account for the confidence difference between a prediction of .99 
and .999 for a $y=1$, which are 0.01 and 0.001 away from 1 but are dramatically
...
\begin{equation}
    L = -[y\log\!\left(p\right) + (1 - y)\log\!\left(1-p\right)]
\end{equation}

\section{Optimization}

% GRADIENT
\subsection{Gradient: $\frac{\partial L}{\partial w_j}$}

As stated before we want to minimize our loss. So how can we do this. 
We want to tune our weights as optimally as possible. Therefore
we use the gradient specifically 
...
We now have the optimal unit of movement of each $w_j$ in order to most efficiently
decrease our loss. 

% N ROWS OF DATA
\subsection{Averaging over a Dataset}

In general we have $d$ columns of observations and $n$ rows of data. 
We can then simply think of each variable described above as a vector. 
Where w is the weight vector in which we add b.
p is the vector of probabilities one for each row $p_i$.
y is the vector of observations of $y_i$.
To get our final gradient we take loss as the average loss of the rows.
...
\begin{equation*}
L_i = -[y_i\log\!\left(p_i\right) + (1 - y_i)\log\!\left(1-p_i\right)]
\end{equation*}

Then to find the partial derivative of the gradient we have 

\begin{equation*}
\frac{\partial L}{\partial w_j} =
\frac{1}{n}\sum_{i=1}^{n}\frac{\partial L_i}{\partial w_j}
\end{equation*}

since the partial derivative of the mean is the mean of the partial derivatives.
Our model reaches its optimum specifically when
...
for all weights $w_j$ and bias $b$.
...
\begin{align*}
\frac{\partial L}{\partial b} = 0
  &\iff \frac{1}{n}\sum_{i=1}^{n}\frac{\partial L_i}{\partial b} = 0 \\
  &\iff \frac{1}{n}\sum_{i=1}^{n} (p_i - y_i) = 0 \\
...
We can then use the model to predict further classifications by training on data
